	...
	<div class="answer-content">
		<span class="answer">
			...
			<?php if ( ( YITH_YWQA()->answer_excerpt_length > 0 ) && ( strlen( $answer->content ) > YITH_YWQA()->answer_excerpt_length ) ) :
				echo substr( $content, 0, YITH_YWQA()->answer_excerpt_length ) . '... <a href="#" rel="nofollow" data-discussion-id="' . $answer->ID . '" class="read-more">' . __( "Read more", 'yith-woocommerce-questions-and-answers' ) . '</a>';
			else:
				echo $content;
			endif;
			?>
	...
	<div class="answer-owner">
		...
		if ( YITH_YWQA()->anonymise_user ) {
            ...
            if ( current_user_can('administrator') || current_user_can('manage_woocommerce') ) {
                ?><a class="edit-on-backend-link" href="<?php echo get_edit_post_link($answer->ID); ?>" target="_blank"><?php _e("Edit on backend", 'yith-woocommerce-questions-and-answers'); ?></a>
            <?php }
		} else {
			...
			<?php if ( $report_abuse ) : ?>
				<div class="answer-abuse">
					<a title="<?php _e( "Report an inappropriate content", 'yith-woocommerce-questions-and-answers' ); ?>" href="#" data-discussion-id="<?php echo $answer->ID; ?>"
					   rel="nofollow" class="report-answer-abuse"><?php _e( "Report", 'yith-woocommerce-questions-and-answers' ); ?></a>
				</div>
			<?php endif; ?>
